...
typedef struct UDPContext {
    ...
    int dest_addr_len;
    int is_connected;
    int wait_for_signal; //added for wait_for_signal
...
static const AVOption options[] = {
    ...
    {"wait_for_signal", "wait for signal to start actually processing data", OFFSET(wait_for_signal), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, D|E },
    { NULL }
};
...
static int udp_open(URLContext *h, const char *uri, int flags)
{
    ...
    if (p) {
        ...
        //Added for wait_for_signal
        if (av_find_info_tag(buf, sizeof(buf), "wait_for_signal", p)) {
            s->wait_for_signal = strtol(buf, NULL, 10);
            if (!HAVE_PTHREAD_CANCEL) {
                av_log(h, AV_LOG_WARNING,
                       "'wait_for_signal' option was set but it is not supported "
                       "on this build (pthread support is required)\n");
	    } else {
	        sigset_t current;
		sigemptyset(&current);//TODO change to current
    ...
    if (s->fifo) {
        ...
        do {
            ...
            if (avail) { // >=size) {
                ...
                if(s->wait_for_signal) {
                    sigset_t pending;
                    sigpending(&pending);
                    if(!sigismember(&pending, SIGUSR1)) {
                        if(avail > av_fifo_space(s->fifo)) {
                           av_log(h, AV_LOG_DEBUG, "Buffer more than half full, flushing datagram\n");
                           flush = 1;
                        } else {
                           pthread_mutex_unlock(&s->mutex);
                    ...
                    av_log(h, AV_LOG_WARNING, "SIGUSR1 received, resuming\n");
                        s->wait_for_signal = 0;
                    }
                }
